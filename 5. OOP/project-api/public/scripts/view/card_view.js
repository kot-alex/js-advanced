export default class CardView {
    ...
    getHtml() {
        return `<figure class="products-card" id="card-${this._data.id}">
                    <div class="products-card-image">
                        <img class="products-card-img" src="images/products/${this._data.image}" alt="product">
                    ...
                    <figcaption class="products-card-info">
                        <a class="products-card-link" href="product.html">
                            <h3 class="products-card-link-heading">${this._data.title}</h3>
                            <p class="products-card-link-text">${this._data.description}</p>
                        </a>
                        <p class="products-card-pricetag">$${this._data.price}</p>
                    </figcaption>
                </figure>`
    }
    ...
    render($container, target = 'beforeend') {
        ...
        if (this._addHandler) {
            const selectedCard = $container.querySelector(`#card-${this._data.id}`);
            const addButton = selectedCard.querySelector('.products-card-overlay-button')
            addButton.addEventListener('click', this._addHandler.bind(this));
        }
    }
